...
# --- Configuration ---
SOURCE_DIR = "/musical-source"
DESTINATION_DIR = "/musical-destination"
LOG_FILE = "/var/log/musical-mover.log" 
...
def get_files_to_move(source_dir):
    """Generates a list of musical files, excluding those marked as TELESYNC."""
    file_list = []
...
def move_files():
    ...
    log_message("-" * 50)
    log_message("🎬 Starting musical file movement process.")
    ...
    # --- 1. Copy Files ---
    log_message("--- Stage 1/3: Copying musicals and creating structure... ---")
    files_to_clean = [] 
...
if __name__ == "__main__":
    log_message(f"musical-mover script starting at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    move_files()
