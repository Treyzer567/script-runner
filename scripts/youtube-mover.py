...
# --- Configuration ---
SOURCE_DIR = "/youtube-source"
DESTINATION_DIR = "/youtube-destination"
LOG_FILE = "/var/log/youtube-mover.log" 
...
def get_files_to_move(source_dir):
    """Generates a list of youtube files, excluding those marked as TELESYNC."""
    file_list = []
    ...
    for root, _, files in os.walk(source_dir):
        ...
        for name in files:
            # --- TELESYNC FILTER ---
            if "TELESYNC" in name:
                log_message(f"Skipping TELESYNC file: {name}")
                continue
...
def move_files():
    ...
    log_message("-" * 50)
    log_message("🎬 Starting youtube file movement process.")
    ...
    # --- 1. Copy Files ---
    log_message("--- Stage 1/3: Copying youtube videos and creating structure... ---")
    files_to_clean = [] 
...
if __name__ == "__main__":
    log_message(f"youtube-mover script starting at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    move_files()
